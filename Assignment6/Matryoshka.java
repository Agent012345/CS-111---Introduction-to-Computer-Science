...
public class Matryoshka {
    ...
    public static void drawDoll(double x, double y, double radius){
       // for (double i = 0; i < n; i++){
            StdDraw.circle(x, y, radius); //Body Circle
            StdDraw.circle(x, y+radius+(.5*radius), radius*.5); //Head Circle
            drawFace(x, y+radius+(.5*radius), radius);
       // }
    }
    ...
    public static void stackDolls(double x, double y, double r, int dolls){
        if (dolls > 0){
            if( dolls == 1){
                ...
                drawDoll(x, y, r);
                double quotient = (double) 5/7;
                stackDolls(x+r+(quotient*r), y, quotient*r, dolls-1);
            }
        }
    }
