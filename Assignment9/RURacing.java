...
public class RURacing {
    ...
    public static int getTotalDistance(char[][] map, int distance) {
    ...
    public static void racer1(char[][] map) {
        ...
        int y = 0;
        int distance = 0;
        distance = getTotalDistance(map, distance);
        moveRacer(x, y, distance, map);
    ...
    public static void racer2(char[][] map) {
        ...
        int distance = 0;
        boolean done = false;
        int totalDistance = getTotalDistance(map, distance);
        distance = 1;
        int distanceTravelled = distance;
        while (!done) {
            int[] coordinates = moveRacer(x, y, distance, map);
            System.out.println("Racer2 is at: " + "(" + coordinates[0] + "," + coordinates[1] + ")");
            distanceTravelled = distanceTravelled + distance;
            distance = distance * 2;
            if (distanceTravelled + distance > totalDistance) {
                distance = (totalDistance - distanceTravelled) + 1;
            }
            x = coordinates[0];
            y = coordinates[1];
            if (x == 0 && y == 0) {
    ...
    public static void racer3(char[][] map) {
        ...
        int distance = 0;
        int totalDistance = getTotalDistance(map, distance);
        for (int i = 0; i <= totalDistance; totalDistance++){
            int[] coordinates = moveRacer(x, y, distance+1, map);
            x = coordinates[0];
            y = coordinates[1];
            System.out.println("Racer3 is at: " + "(" + coordinates[0] + "," + coordinates[1] + ")");
            if (x == 0 && y == 0) {
                break;
            }
        }
    ...
    public static void racer4(char[][] map) { // starting X and starting Y
        ...
        for (int i = 0; i <= totalDistance; i++){
            ...
            x = coordinates[0];
            y = coordinates[1];
            int decrementedDistance = totalDistance - 1;
            if (x == 0 && y == 0) {
                System.out.println("Racer4 is at: " + "(" + coordinates[0] + "," + coordinates[1] + ")");
                break;
            }
            for (int j = decrementedDistance; j > 0; j--) {
                System.out.println("Racer4 is at: " + "(" + coordinates[0] + "," + coordinates[1] + ")");
            }
        }
    }
